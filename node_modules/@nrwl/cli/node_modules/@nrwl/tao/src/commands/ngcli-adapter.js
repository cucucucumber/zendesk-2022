...
class NxScopedHost extends core_1.virtualFs.ScopedHost {
    constructor(root) {
        ...
        this.__readWorkspaceConfiguration = (configFileName, overrides) => {
            return super.exists('nx.json').pipe((0, operators_1.switchMap)((nxJsonExists) => (!nxJsonExists // if no nxJson, let it be undefined
                ? ((overrides === null || overrides === void 0 ? void 0 : overrides.workspace) || super.read(configFileName)).pipe((0, operators_1.map)((x) => [x]))
                ...
                ])).pipe((0, operators_1.switchMap)(([w, n]) => {
                var _a, _b, _c;
                try {
                    // parse both from json, nxJson may be null
                    const workspaceJson = (0, json_1.parseJson)(Buffer.from(w).toString());
                    const nxJson = n
                        ? (0, json_1.parseJson)(Buffer.from(n).toString())
                        : null;
    ...
    context(path) {
        if (isWorkspaceConfigPath(path)) {
            return super.exists('/angular.json').pipe((0, operators_1.switchMap)((isAngularJson) => {
    ...
    writeWorkspaceConfiguration(context, content) {
        ...
        if (context.isNewFormat) {
            try {
                ...
                if (formatted) {
                    ...
                    return (0, rxjs_1.concat)(this.writeWorkspaceConfigFiles(context, workspaceJson), cli || generators || defaultProject
                    ...
                    return (0, rxjs_1.concat)(this.writeWorkspaceConfigFiles(context.actualConfigFileName, angularJson), cli || schematics
        ...
        return (0, rxjs_1.concat)(this.writeWorkspaceConfigFiles(context, angularJson), this.__saveNxJsonProps({
            cli,
            defaultProject,
            generators: schematics || generators,
        }));
    ...
    writeWorkspaceConfigFiles({ actualConfigFileName: workspaceFileName, isNewFormat }, config) {
        ...
        for (const [project, projectConfig] of projects) {
            if (projectConfig.configFilePath) {
                ...
                writeObservable = writeObservable
                    ? (0, rxjs_1.concat)(writeObservable, projectJsonWrite)
                    : projectJsonWrite;
        ...
        return writeObservable
            ? (0, rxjs_1.concat)(writeObservable, workspaceJsonWrite)
            : workspaceJsonWrite;
    }
    resolveInlineProjectConfigurations(config) {
        ...
        Object.entries((_a = config.projects) !== null && _a !== void 0 ? _a : {}).forEach(([project, projectConfig]) => {
            if (typeof projectConfig === 'string') {
                ...
                observable = observable ? (0, rxjs_1.concat)(observable, next) : next;
            }
        });
        ...
        if (isWorkspaceConfigPath(path)) {
            ...
            try {
                return this.__readWorkspaceConfiguration(match.path, {
                    workspace: (0, rxjs_1.of)(match.content),
                    nx: nxJsonChange ? (0, rxjs_1.of)(nxJsonChange.content) : null,
                });
            }
            catch (e) {
                return super.read(path);
